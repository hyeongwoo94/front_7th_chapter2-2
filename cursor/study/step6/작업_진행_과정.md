...
Teacher가 단계 6을 구현한 과정을 간결하게 정리합니다.

## 📋 작업 순서

1. **기존 코드 구조 파악** → hooks.ts 함수 시그니처 확인
2. **useState 구현** → 상태 관리 훅
3. **useEffect 구현** → 사이드 이펙트 훅
4. **cleanupUnusedHooks 구현** → 미사용 훅 정리
5. **flushEffects 구현** → 예약된 이펙트 실행

---

## 🔧 구현 내용

### 1. `useState` (`core/hooks.ts`)
...
### 3. `cleanupUnusedHooks` (`core/hooks.ts`)
